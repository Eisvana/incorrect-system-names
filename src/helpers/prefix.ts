...
function getRegionNumber(glyphs: string): number {
  const regionGlyphsSubstring = glyphs.slice(4); // oxlint-disable-line no-magic-numbers
  const regionIndex = regionGlyphs.indexOf(regionGlyphsSubstring);
  return regionIndex + 1;
}

export function getPrefix(glyphs: string): string {
  const systemIndexString = glyphs.slice(1, 4); // oxlint-disable-line no-magic-numbers
